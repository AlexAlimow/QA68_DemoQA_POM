...
public class SelectPage extends BasePage {
    ...
    public SelectPage oldStileSelect(String color) {
    ...
    public SelectPage verifyColor() {
        String fistSelectedOption = new Select(oldSelectMenu).getFirstSelectedOption().getText();
        Assertions.assertTrue(shouldHaveText(oldSelectMenu, fistSelectedOption, 5));
        return this;
    }
    ...
    public SelectPage MultiSelect(String[] colors) {
        for (String text : colors) {
            ...
            selectInput.sendKeys(Keys.ENTER); // Делает Enter ( это Enum)
        }
        click(space); // а тут просто клик на экран, чтобы закрыть выпадающее меню
        return this;
    }
